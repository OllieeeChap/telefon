...
class RotaryDial:
    ...
    # We'll be reading BCM GPIO 17 (pin 7 on board)
    pin_rotary = 17

    # We'll be reading on/off hook events from BCM GPIO 3
    pin_onhook = 21

    # After 500ms, we assume the rotation is done and we get
    # the final digit. 
    digit_timeout = 0.5
    ...
    def __init__(self):
        ...
        GPIO.add_event_detect(self.pin_onhook, GPIO.BOTH, callback = self.HookEvent, bouncetime=500)
   ...
   #     time.sleep(0.002)
    ...
    def RegisterCallback(self, NumberCallback, OffHookCallback, OnHookCallback, OnVerifyHook):
        ...
        input = not GPIO.input(self.pin_onhook)
        if input:
            self.OffHookCallback()
        else:
            self.OnHookCallback()
